import { type HersheyGlyph, scriptSimple, scriptComplex } from "../fonts/hershey-data";
import type { CurveMode, FontVariant, SmoothingOptions } from "../types";
...
function fitCurveToPath(
  ...
  // The root app does not yet vendor the fit-curve package, so the MVP
  // falls back to the existing Catmull-Rom path generation.
  void error;
  return catmullRom(points, 4, includeMoveTo);
}
...
export const layoutTextSegmented = layoutText;
